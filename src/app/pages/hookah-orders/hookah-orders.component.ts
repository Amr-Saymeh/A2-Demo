...
@Component({
  selector: 'app-hookah-orders',
  standalone: true,
  imports: [],
  templateUrl: './hookah-orders.component.html',
  styleUrls: ['./hookah-orders.component.css']
})
export class HookahOrdersComponent implements OnInit, OnDestroy {
  restaurantId = '';
  hookahOrders: { tableKey: string; tableName: string; orderKey: string; order: Order }[] = [];
    ...
    this.tablesUnsub = onValue(tablesRef, snapshot => {
      ...
      Object.keys(data).forEach(tableKey => {
        ...
        Object.keys(tableOrders).forEach(orderKey => {
          ...
          if (section === 'hookah' && ord.state !== 'done'&& ord.state !== 'unordered') {
            orders.push({ tableKey, tableName: table.name || tableKey, orderKey, order: ord });
          }
        });
      });

      this.ngZone.run(() => (this.hookahOrders = orders));
    });
  }
