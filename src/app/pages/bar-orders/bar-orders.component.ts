...
export class BarOrdersComponent implements OnInit, OnDestroy {
  restaurantId = '';
  barOrders: { tableKey: string; tableName: string; orderKey: string; order: Order }[] = [];
    ...
    this.tablesUnsub = onValue(tablesRef, snapshot => {
      ...
      Object.keys(data).forEach(tableKey => {
        ...
        Object.keys(tableOrders).forEach(orderKey => {
          ...
          // Only show bar orders that are NOT done
          if (section === 'bar' && ord.state !== 'unordered' && ord.state !== 'done') {
            orders.push({ tableKey, tableName: table.name || tableKey, orderKey, order: ord });
          }
        });
      });

      this.ngZone.run(() => (this.barOrders = orders));
    });
  }
